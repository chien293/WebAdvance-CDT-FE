...
const GradeStructureBoard = ({ classId }) => {
  ...
  const handleFinalOk = async () => {
    setFinalRowData((prev) =>
      prev.finalScore === 0 ? (prev.finalScore = 1) : (prev.finalScore = 0)
    );
    try {
      await axios.post(
    ...
    const updatedData = gradeStructure.map((item) =>
      item.id === finalRowData.id
        ? {
            ...item,
            percentage: finalRowData.percentage,
            value: finalRowData.value,
          }
        : item
    );
  ...
  const handleEditOk = async () => {
    ...
    const updatedData = gradeStructure.map((item) =>
      item.id === editRowData.id
        ? {
            ...item,
            percentage: editRowData.percentage,
  ...
  const handleOk = () => {
    ...
    axios.post(
      API_URL + "/class/addGradeStructure",
      {
        idClass: classId,
        percentage: newRowData.percentage,
        value: newRowData.value,
  ...
  const handleDeleteOk = () => {
    const deleteIndex = gradeStructure.findIndex(
    ...
    axios.post(
      API_URL + "/class/deleteGradeStructure",
      {
        idClass: classId,
        id: deleteRowData.id,
      ...
      <DndProvider backend={HTML5Backend}>
        <Table
          ...
          components={{
            body: {
              row: (props) => (
                <DragableBodyRow {...props} moveRow={moveRowRef.current} />
              ),
            },
          }}
          onRow={(record, index) => ({
